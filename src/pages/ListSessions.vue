<template>
    <div class="m-10">
        ...
        <div v-else class="grid grid-cols-3 w-full gap-4 p-5">
            <SessionCard v-for="session in store.sessions" :key="session.id" :id="session.id"
                :trainingName="session.trainingName" :sessionName="session.sessionName"
                :startingDate="session.startingDate" :endingDate="session.endingDate"
