<template>
  ...
  <div class="w-1/2 max-w-sm p-4 border border-gray-300 rounded-lg shadow-sm relative">
    <label v-if="!fileName"
      ...
      <input type="file" class="hidden" @change="handleFileUpload" accept="image/*, .pdf, .docx" />
    ...
    <div v-if="fileName" class="relative flex items-center justify-center w-full h-32 bg-gray-100 rounded-lg p-2">
      <!-- Remove button -->
      ...
      <!-- File preview -->
      <img v-if="isImage" :src="previewUrl" alt="Uploaded Image" class="h-full max-w-full object-cover rounded-md" />
...
const handleFileUpload = (event) => {
  ...
  // emit("errorMessage", ""); // Clear previous error messages
